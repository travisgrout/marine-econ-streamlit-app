...
# Function to handle button clicks for Reviewer Displays
...
with rev_cols[0]:
    ...
    st.button("Compare to ENOW", on_click=update_mode, args=("Compare",), use_container_width=True, type="primary" if is_selected else "secondary", help="Compare to original ENOW", key="compare_button")
with rev_cols[1]:
    is_selected = st.session_state.plot_mode == button_map["Error Analysis"]
    st.button("Error Analysis", on_click=update_mode, args=("Error Analysis",), use_container_width=True, type="primary" if is_selected else "secondary", help="Analyze differences between Open ENOW and original ENOW", key="error_button")

# --- END: REVISED AND CORRECTED SIDEBAR LAYOUT ---
